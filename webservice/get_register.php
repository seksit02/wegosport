...
if (!$conn) {
    die(json_encode(array("result" => 0, "message" => "Database connection failed: " . mysqli_connect_error())));
}
?>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($json_data) {
        ...
        if ($query) {
            ...
            $result = 0;
            $message = "มีข้อมูลซ้ำในระบบหรือข้อผิดพลาดอื่น ๆ: " . mysqli_error($conn);
            $datalist[] = null;
        }

        echo json_encode(array("result" => $result, "message" => $message, "datalist" => $datalist));
    } else {
        echo json_encode(array("result" => 0, "message" => "Invalid JSON data received"));
    }
